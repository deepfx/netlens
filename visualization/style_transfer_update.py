...
#         self.loss = torch.sum(torch.abs(x_diff)) + torch.sum(torch.abs(y_diff))
#         return input
...
class StyleTransferModule(LayeredModule):
    ...
    def run_style_transfer(self, input_img, optimizer_class=optim.LBFGS, num_steps=300, style_weight=100000, content_weight=1, tv_weight=1e-3, verbose=True):
